...
const LineChartComponent: React.FC<CombinedChartProps> = ({
  title,
  chartType,
  filterType,
  alert,
  ...
  const generatedData = generateChartData(chartType, filterType);
  const chartData = data?.data || generatedData;
          ...
          <div key={key} className="flex flex-row items-center">
            <div className="flex flex-col items-center">
              <div className="flex items-center">
                <span
                  className="sm:w-2 sm:h-2 w-1 h-1 rounded-full"
                  style={{ backgroundColor: selectedColors[key] }}
                ></span>
                <span className="ml-2 text-[12px] sm:text-sm">{key}</span>
              </div>
              <span className="text-[12px] sm:text-lg">{data?.sum?.[key]}</span>
            </div>
            {index !== array.length - 1 && (
              <div className="w-[1px] h-10 bg-gray-500 mx-4"></div>
            )}
          </div>
        ))}
      </div>
