...
//             {camIds.map((camId) => (
//                 <div key={camId} className="text-left flex flex-col h-full gap-y-14 relative">
...
const CameraStream = ({ camIds }: { camIds: string[] }) => {
    ...
    useEffect(() => {
        ...
        camIds.forEach((camId) => {
            ...
            ws.onopen = () => {
                console.log(`Connected to WebSocket server for ${camId}`);
                activeCams.push(camId);
                setConnectedCams([...activeCams]);
            };
            ...
            ws.onclose = () => {
                console.log(`Disconnected from WebSocket server for ${camId}`);
                activeCams = activeCams.filter((id) => id !== camId);
                setConnectedCams([...activeCams]);
            };
        });
    ...
    return (
        <div>
            {connectedCams.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start max-h-[900px] h-full">
                    {connectedCams.map((camId) => (
                        <div key={camId} className="text-left flex flex-col h-full gap-y-14 relative">
                            <div className="relative">
                                <img src={frames[camId]} alt={`Camera ${camId}`} className="w-full h-full max-h-[500px] object-contain" />
                                <p className="absolute bottom-4 left-4 bg-gray-400 text-white text-sm px-2 py-1 rounded-md">{camId}</p>
                                <p className={`absolute top-4 left-4 text-white text-md sm:text-lg font-bold sm:px-4 sm:py-2 px-3 py-1 rounded-md ${personDetected[camId] ? 'bg-red-500' : 'bg-green-500'}`}>
                                    {personDetected[camId] ? "Có người!" : "Không có người"}
                                </p>
                            </div>
                            <CameraStatus
                                totalAnimals={10010}
                ...
                <div className="text-center text-lg text-gray-500 font-semibold py-10">Không có cam nào kết nối</div>
            )} 
        </div>
    );
};
