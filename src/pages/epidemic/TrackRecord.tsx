    ...
    return (
        <div>
            ...
            <DataTable className="p-2 bg-[#F3F7F5] rounded-[20px]" ref={dt} value={products} selection={selectedProducts} 
                    ...
                    <Column className="bg-[#F3F7F5]"  field="_id" header="_id" sortable style={{ minWidth: '1rem' }}></Column>
                    <Column className="bg-[#F3F7F5]"  field="species" header="Giống loài" sortable style={{ minWidth: '1rem' }}></Column>
                    <Column className="bg-[#F3F7F5]" field="barn"  header="Khu vực cách ly"  sortable style={{ minWidth: '4rem' }}></Column>
                    <Column   className="bg-[#F3F7F5]"  field="detectionDate"   header="Ngày phát hiện" sortable   style={{ minWidth: '4rem' }}
                        body={(rowData) => rowData.detectionDate ? new Date(rowData.detectionDate).toLocaleDateString() : "Chưa có"}
                    ></Column>
                    <Column className="bg-[#F3F7F5]"  field="diseaseType" header="Loại bệnh" sortable style={{ minWidth: '1rem' }}></Column>
                    <Column className="bg-[#F3F7F5]"  field="entryDate" header="Ngày tái nhập đàn" 
                        body={(rowData) => rowData.entryDate ? new Date(rowData.entryDate).toLocaleDateString() : "Chưa có"}
                        sortable style={{ minWidth: '1rem' }}></Column>
                    <Column className="bg-[#F3F7F5]" field="treatmentHistory"  header="Lịch sử điều trị" 
                     body={(rowData) => (
                      <button
